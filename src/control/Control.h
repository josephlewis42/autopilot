...
public:
	...
	inline heli::Controller_Mode get_controller_mode() const {std::lock_guard<std::mutex> lock(controller_mode_lock); return controller_mode;}
	...
	blas::vector<double> get_reference_attitude() const {std::lock_guard<std::mutex> lock(reference_attitude_lock); return reference_attitude;}

	/// set trajectory type
	void set_trajectory_type(const heli::Trajectory_Type trajectory_type);
	/// get the trajectory type
	heli::Trajectory_Type get_trajectory_type() const {std::lock_guard<std::mutex> lock(trajectory_type_lock); return trajectory_type;}
...
private:
	...
	void set_reference_attitude(const blas::vector<double>& reference_attitude) {std::lock_guard<std::mutex> lock(reference_attitude_lock); this->reference_attitude = reference_attitude;}


	/// threadsafe set reference_position
	void set_reference_position(const blas::vector<double>& position) {std::lock_guard<std::mutex> lock(reference_position_lock); reference_position = position;}
