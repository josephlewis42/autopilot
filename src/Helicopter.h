...
public:
  ...
  double get_mass() const {std::lock_guard<std::mutex> lock(mass_lock); return mass;}
  /// get gravity
  double get_gravity() const {return gravity;}
  /// return the main rotor hub offset
  blas::vector<double> get_main_hub_offset() {std::lock_guard<std::mutex> lock(main_hub_offset_lock); return main_hub_offset;}
  /// return the tail rotor hub offset
  blas::vector<double> get_tail_hub_offset() {std::lock_guard<std::mutex> lock(tail_hub_offset_lock); return tail_hub_offset;}
  /// return the inertia matrix
  blas::banded_matrix<double> get_inertia() {std::lock_guard<std::mutex> lock(inertia_lock); return inertia;}
...
private:
  ...
  /// set the mass
  inline void set_mass(const double mass) {{std::lock_guard<std::mutex> lock(mass_lock); this->mass = mass;} message() << "Mass set to " << mass;}
  ...
  inline void set_main_hub_offset(const blas::vector<double>& main_hub_offset) {std::lock_guard<std::mutex> lock(main_hub_offset_lock); this->main_hub_offset = main_hub_offset;}
  /// set the body x main_hub_offset
  inline void set_main_hub_offset_x(const double& main_hub_offset_x) {{std::lock_guard<std::mutex> lock(main_hub_offset_lock); main_hub_offset(0) = main_hub_offset_x;} message() << "Main Hub Offset x set to " << main_hub_offset_x;}
  /// set the body y main_hub_offset
  inline void set_main_hub_offset_y(const double& main_hub_offset_y) {{std::lock_guard<std::mutex> lock(main_hub_offset_lock); main_hub_offset(1) = main_hub_offset_y;} message() << "Main Hub Offset y set to " << main_hub_offset_y;}
  /// set the body z main_hub_offset
  inline void set_main_hub_offset_z(const double& main_hub_offset_z) {{std::lock_guard<std::mutex> lock(main_hub_offset_lock); main_hub_offset(2) = main_hub_offset_z;} message() << "Main Hub Offset z set to " << main_hub_offset_z;}
  ...
  inline void set_tail_hub_offset(const blas::vector<double>& tail_hub_offset) {std::lock_guard<std::mutex> lock(tail_hub_offset_lock); this->tail_hub_offset = tail_hub_offset;}
  /// set the body x tail_hub_offset
  inline void set_tail_hub_offset_x(const double& tail_hub_offset_x) {{std::lock_guard<std::mutex> lock(tail_hub_offset_lock); tail_hub_offset(0) = tail_hub_offset_x;} message() << "Tail Hub Offset x set to " << tail_hub_offset_x;}
  /// set the body y tail_hub_offset
  inline void set_tail_hub_offset_y(const double& tail_hub_offset_y) {{std::lock_guard<std::mutex> lock(tail_hub_offset_lock); tail_hub_offset(1) = tail_hub_offset_y;} message() << "Tail Hub Offset y set to " << tail_hub_offset_y;}
  /// set the body z tail_hub_offset
  inline void set_tail_hub_offset_z(const double& tail_hub_offset_z) {{std::lock_guard<std::mutex> lock(tail_hub_offset_lock); tail_hub_offset(2) = tail_hub_offset_z;} message() << "Tail Hub Offset z set to " << tail_hub_offset_z;}
  ...
  inline void set_inertia(const blas::banded_matrix<double>& inertia) {std::lock_guard<std::mutex> lock(inertia_lock); this->inertia = inertia;}
  /// set the inertia in the body x
  inline void set_inertia_x(const double& jx) {{std::lock_guard<std::mutex> lock(inertia_lock); inertia(0,0) = jx;} message() << "Inertia x set to " << jx;}
  /// set the inertia in the body y
  inline void set_inertia_y(const double& jy) {{std::lock_guard<std::mutex> lock(inertia_lock); inertia(1,1) = jy;} message() << "Inertia y set to " << jy;}
  /// set the inertia in the body z
  inline void set_inertia_z(const double& jz) {{std::lock_guard<std::mutex> lock(inertia_lock); inertia(2,2) = jz;} message() << "Inertia z set to " << jz;}
