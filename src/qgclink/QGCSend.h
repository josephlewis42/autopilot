...
private:
    ...
    inline heli::AUTOPILOT_MODE get_servo_source() const
    {
        return servo_source;
    }
    inline void set_servo_source(heli::AUTOPILOT_MODE servo_source)
    {
        this->servo_source = servo_source;
    }

    /// store the current pilot mode (threadsafe)
    std::atomic<heli::PILOT_MODE> pilot_mode;
    inline heli::PILOT_MODE get_pilot_mode() const
    {
        return pilot_mode;
    }
    inline void set_pilot_mode(heli::PILOT_MODE pilot_mode)
    {
        this->pilot_mode = pilot_mode;
    }

    /// store the current gx3 filter state
    std::atomic<IMU::GX3_MODE> filter_state;
    inline IMU::GX3_MODE get_filter_state() const
    {
        return filter_state;
    }
    inline void set_filter_state(IMU::GX3_MODE filter_state)
    {
    ...
    inline heli::Controller_Mode get_control_mode() const
    {
        return control_mode;
    }
    inline void set_control_mode(heli::Controller_Mode control_mode)
    {
    ...
    inline void clear_gx3_new_message()
    {
        gx3_new_message = false;
    }
    /// threadsafe set gx3_new_message
    inline void set_gx3_new_message()
    {
        gx3_new_message = true;
    }
    /// threadsafe get gx3_new_message
    inline bool get_gx3_new_message() const
    {
        return gx3_new_message;
    }
    ...
    inline std::string get_gx3_message() const
    {
        std::lock_guard<std::mutex> lock(gx3_message_lock);
        return gx3_message;
    }
    /// threadsafe set gx3_messsage (can be slot)
    inline void set_gx3_message(std::string message)
    {
        std::lock_guard<std::mutex> lock(gx3_message_lock);
        gx3_message = message;
        set_gx3_new_message();
    ...
    inline bool get_attitude_source() const
    {
        return attitude_source;
    }
    inline void set_attitude_source(bool source)
    {
    ...
    inline void message_queue_push(const std::string& message)
    {
        std::lock_guard<std::mutex> lock(message_queue_lock);
        message_queue.push(message);
    }
    /// threadsafe empty test
    inline bool message_queue_empty() const
    {
        std::lock_guard<std::mutex> lock(message_queue_lock);
